...
import { DatePicker, Descriptions, Input, Tag } from 'antd';
...
const PaymentList: React.FC<PaymentListProps> = ({ filter }) => {
    ...
    const paymentList: Payment[] = data?.payload || [];
    useEffect(() => {
    ...
    const filterData = paymentList.filter((f) => f.userName.toLowerCase().includes(debouncedValue.toLowerCase()));
