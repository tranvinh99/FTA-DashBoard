    ...
    return (
        <FormWrapper methods={methods}>
            <form onSubmit={methods.handleSubmit(onSubmit)} className="flex flex-col w-full gap-2">
                ...
                <TableBuilder<OrderTien>
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            width: 400,
                            key: 'shipAddress',
                            render: ({ ...props }: OrderTien) => <p className="m-0">{props.stationResponse?.name}</p>,
                        },
                        {
                ...
                />
                <Button
                    // Đảm bảo nút chỉ có thể nhấn khi có ít nhất một đơn hàng được chọn
                    disabled={selectedRowKeys.length === 0 || data?.payload.length === 0}
                    type="primary"
                    onClick={() => {
